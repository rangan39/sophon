...
export function SophonModelSelector({ disabled = false, modelId, onSelect }: SophonModelSelectorProps) {
  ...
  function handleSearchKeyDown(event: React.KeyboardEvent<HTMLInputElement>) {
    if (event.key === "ArrowDown") {
      ...
      event.preventDefault();
      setActiveIndex(Math.max(0, filteredModels.length - 1));
    ...
    <Dialog.Root onOpenChange={changeOpen} open={open}>
      <Dialog.Trigger asChild>
        <button
          ...
          className="relative flex h-7 min-w-[108px] max-w-[148px] items-center gap-1.5 border border-white/[.12] bg-white/[.04] px-2 font-mono uppercase text-white/65 transition-colors after:absolute after:-inset-y-2 after:content-[''] hover:border-sophon-signal-bright/50 hover:bg-white/[.06] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sophon-warning disabled:cursor-not-allowed disabled:opacity-45"
          disabled={disabled}
          type="button"
        >
          <span aria-hidden="true" className="font-serif text-[11px] normal-case text-sophon-signal-soft">μ</span>
      ...
      <Dialog.Portal>
        <Dialog.Overlay className="sophon-dialog-overlay fixed inset-0 z-50 bg-black/70 backdrop-blur-[2px]" />
        <Dialog.Content className="sophon-dialog-content fixed left-1/2 top-1/2 z-50 flex max-h-[min(680px,calc(100dvh-2rem))] w-[min(540px,calc(100vw-2rem))] -translate-x-1/2 -translate-y-1/2 flex-col border border-white/[.13] bg-sophon-panel shadow-[0_30px_100px_rgb(0_0_0/.65)] focus:outline-none">
          <div className="flex items-start justify-between border-b border-white/[.08] px-4 py-3">
            ...
            <Dialog.Close className="grid size-9 place-items-center border border-white/[.08] font-mono text-sm text-white/35 transition-colors hover:border-sophon-signal-bright/40 hover:text-white/70 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" aria-label="Close model selector">×</Dialog.Close>
          </div>

          <div className="border-b border-white/[.08] p-3">
            <label className="flex h-10 items-center gap-2 border border-white/[.1] bg-white/[.025] px-3 focus-within:border-sophon-signal-bright/50">
              <span aria-hidden="true" className="font-serif text-sm text-sophon-signal-soft">λ</span>
              <input
                aria-activedescendant={filteredModels[activeIndex] ? `sophon-model-option-${filteredModels[activeIndex]?.id}` : undefined}
                aria-autocomplete="list"
                aria-controls="sophon-model-options"
                aria-expanded="true"
            ...
            <span aria-live="polite" className="sr-only">{filteredModels.length} models available</span>
          </div>

          <div aria-label="Available models" className="min-h-0 overflow-y-auto p-2" id="sophon-model-options" role="listbox">
            {filteredModels.length ? filteredModels.map((model, index) => {
              ...
              return (
                <button
                  ...
                  className={`grid w-full grid-cols-[minmax(0,1fr)_auto] gap-4 border px-3 py-3 text-left transition-colors focus-visible:outline-none ${active ? "border-sophon-signal-bright/35 bg-sophon-signal/[.07]" : "border-transparent hover:border-white/[.08] hover:bg-white/[.025]"}`}
                  id={`sophon-model-option-${model.id}`}
                  key={model.id}
                ...
                >
                  <span className="min-w-0">
                    <span className="flex items-center gap-2">
                      ...
                      {selected ? <span aria-hidden="true" className="font-serif text-xs text-sophon-signal-soft">Δ</span> : null}
                    </span>
                  ...
                  <span className={`font-mono text-[8px] uppercase tracking-[0.12em] ${model.verification === "verified" ? "text-sophon-verified" : "text-sophon-warning"}`}>{model.verification}</span>
                </button>
              );
            }) : (
              <div aria-disabled="true" aria-selected="false" className="px-4 py-12 text-center font-mono text-[10px] uppercase tracking-[0.14em] text-white/25" role="option">No matching models</div>
            )}
          </div>
        </Dialog.Content>
      </Dialog.Portal>
